...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        act_nodes = []
        pre_state_nodes = self.s_levels[level]

        for action in self.all_actions:
            act_node = PgNode_a(action)
            if act_node.prenodes.issubset(pre_state_nodes):
                act_nodes.append(act_node)

        for act_node in act_nodes:
            for state_node in pre_state_nodes:
                act_node.parents.add(state_node)
                state_node.children.add(act_node)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect in node_a1.action.precond_pos:
            if effect in node_a2.action.effect_rem:
                return True
        
        for effect in node_a1.action.precond_neg:
            if effect in node_a2.action.effect_add:
                return True

        for effect in node_a2.action.precond_pos:
            if effect in node_a1.action.effect_rem:
                return True

        for effect in node_a2.action.precond_neg:
